...
function Navbar({ connectMetaMask, metaMaskConnected, disconnectMetaMask, account, isTronWalletConnected }) {
    ...
    const onChangeLang = (code, label, img) => {
    ...
    return (
        <div className={`px-2 flex w-full items-center justify-between fixed z-40 top-0 left-0 h-28 md:pr-8 dark:bg-[rgba(255,255,255,0)] backdrop-blur-[30px] shadow-[0_3px_6px_3px_rgba(0,0,0,0.4)] transition-all duration-300 ${fontClass}`}>
            ...
            <div className="flex items-center">
                {metaMaskConnected ? (
                    <button onClick={disconnectMetaMask} className={`w-28 md:w-32 flex justify-center bg-[#5170fd] text-white text-[16px] font-bold hover:scale-105 transition-transform duration-300 ${fontClass}`}>
                        {`Connected: ${account.slice(0, 6)}...${account.slice(-4)}`}
                    </button>
                ) : (
                    isTronWalletConnected ? (
                        ...
                        <button onClick={connectMetaMask} className={`w-28 md:w-32 flex justify-center bg-[#5170fd] text-white text-[16px] font-bold hover:scale-105 transition-transform duration-300 ${fontClass}`}>
                            Connect MetaMask
                        </button>
                    )
                )}
